/*
 * This file was automatically generated by EvoSuite
 * Thu May 14 13:32:24 UTC 2020
 */

package org.xwiki.model.reference;

import org.junit.Test;
import static org.junit.Assert.*;
import static org.evosuite.runtime.EvoAssertions.*;
import org.evosuite.runtime.EvoRunner;
import org.evosuite.runtime.EvoRunnerParameters;
import org.junit.runner.RunWith;
import org.xwiki.model.EntityType;
import org.xwiki.model.reference.EntityReference;

@RunWith(EvoRunner.class) @EvoRunnerParameters(useVFS = true, useJEE = true) 
public class EntityReference_ESTest extends EntityReference_ESTest_scaffolding {

  @Test(timeout = 4000)
  public void test0()  throws Throwable  {
      String string0 = "-)fORl$`BI(a_DmT";
      EntityType entityType0 = EntityType.OBJECT_PROPERTY;
      EntityReference entityReference0 = new EntityReference("-)fORl$`BI(a_DmT", entityType0);
      entityReference0.hashCode();
      entityReference0.appendParent(entityReference0);
      entityReference0.hashCode();
      entityReference0.getReversedReferenceChain();
      String string1 = null;
      // Undeclared exception!
      entityReference0.setName((String) null);
  }
}
