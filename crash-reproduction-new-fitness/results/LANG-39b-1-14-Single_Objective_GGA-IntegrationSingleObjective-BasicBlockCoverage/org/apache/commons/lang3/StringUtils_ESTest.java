/*
 * This file was automatically generated by EvoSuite
 * Sat May 16 21:06:41 UTC 2020
 */

package org.apache.commons.lang3;

import org.junit.Test;
import static org.junit.Assert.*;
import static org.evosuite.runtime.EvoAssertions.*;
import org.apache.commons.lang3.StringUtils;
import org.evosuite.runtime.EvoRunner;
import org.evosuite.runtime.EvoRunnerParameters;
import org.junit.runner.RunWith;

@RunWith(EvoRunner.class) @EvoRunnerParameters(useVFS = true, useJEE = true) 
public class StringUtils_ESTest extends StringUtils_ESTest_scaffolding {

  @Test(timeout = 4000)
  public void test0()  throws Throwable  {
      StringUtils.substringAfter("Minimum abbreviation width with offset is 7", "p{InCombiningDiacriticalMarks}+");
      StringUtils.contains("Minimum abbreviation width with offset is 7", "p{InCombiningDiacriticalMarks}+");
      String[] stringArray0 = new String[5];
      stringArray0[0] = "Minimum abbreviation width with offset is 7";
      stringArray0[1] = "Minimum abbreviation width with offset is 7";
      stringArray0[1] = "p{InCombiningDiacriticalMarks}+";
      stringArray0[3] = "";
      stringArray0[4] = "Minimum abbreviation width with offset is 7";
      // Undeclared exception!
      StringUtils.replaceEach("Minimum abbreviation width with offset is 7", stringArray0, stringArray0);
  }
}
