/*
 * This file was automatically generated by EvoSuite
 * Sun May 17 19:17:22 UTC 2020
 */

package com.xpn.xwiki.objects.classes;

import org.junit.Test;
import static org.junit.Assert.*;
import static org.evosuite.runtime.EvoAssertions.*;
import com.xpn.xwiki.objects.classes.BaseClass;
import com.xpn.xwiki.objects.classes.TextAreaClass;
import org.evosuite.runtime.EvoRunner;
import org.evosuite.runtime.EvoRunnerParameters;
import org.junit.runner.RunWith;

@RunWith(EvoRunner.class) @EvoRunnerParameters(useVFS = true, useJEE = true) 
public class BaseClass_ESTest extends BaseClass_ESTest_scaffolding {

  @Test(timeout = 4000)
  public void test0()  throws Throwable  {
      BaseClass baseClass0 = new BaseClass();
      String string0 = "#=qA5nJR;Hz/b1~/+%";
      int int0 = 86;
      boolean boolean0 = true;
      baseClass0.addStaticListField("#=qA5nJR;Hz/b1~/+%", "#=qA5nJR;Hz/b1~/+%", 86, true, "#=qA5nJR;Hz/b1~/+%");
      String string1 = "validationScript";
      int int1 = 0;
      TextAreaClass.EditorType textAreaClass_EditorType0 = TextAreaClass.EditorType.PURE_TEXT;
      TextAreaClass.ContentType textAreaClass_ContentType0 = TextAreaClass.ContentType.VELOCITY_CODE;
      // Undeclared exception!
      baseClass0.addTextAreaField("#=qA5nJR;Hz/b1~/+%", "validationScript", 0, 0, textAreaClass_EditorType0, textAreaClass_ContentType0);
  }
}
