...
reproductionRateEffectSize <- getOddsRatioReproduction("WeightedSum") %>%
  mutate(winner = ifelse(oddsratio > 1,"WeightedSum+BBC","WeightedSum"))
...
resultsForReproductionCount$cat <- ifelse(resultsForReproductionCount$cat == "WeightedSum","WeightedSum","WeightedSum+BBC")
