...
timeEffectSize <- getTimeEffectSizes("WeightedSum") %>%
  mutate(winner = ifelse(VD.estimate.category == "< 0.5","WeightedSum+BBC","WeightedSum"))


efficiency_abstract <- data.frame('Winner'=NA, 'count'=NA,"VD_magnitude"=NA)
efficiency_abstract <- efficiency_abstract[0,]

for (ca in c("WeightedSum","WeightedSum+BBC")){
...
mutateTimeEffectSize$cat.alg1 <- "WeightedSum+BBC"
