...
BBC_CONFIGS = c("WeightedSum+old-BBC","WeightedSum+BBC")
# For WS
...
configurations <- c("WeightedSum","WeightedSum+BBC")
...
improvementSummary <- mutateTimeEffectSize %>%
  filter(cat.alg1 == "WeightedSum+BBC" & cat.alg2 == "WeightedSum") %>%
  group_by(cat.alg1) %>%
...
configurations <- c("WeightedSum","WeightedSum+old-BBC")
...
ggsave("figures/significant-efficiency-ws-oldvsnone.pdf", width = 5.5, height = 4)
...
configurations <- c("WeightedSum+old-BBC","WeightedSum+BBC")
...
p <- ggplot(data=efficiency_abstract, aes(x=Winner, y=count, fill=VD_magnitude)) +
  ...
  ylab("# of crashes") + xlab("Configurations") +
  theme(legend.position = "top", legend.text=element_text(size=14), axis.text.x=element_text(size=14))
ggsave("figures/significant-efficiency-ws-oldvsbbc.pdf", width = 6.5, height = 4)
...
ggsave("figures/improvement-efficiency-ws.pdf", p, width = 7, height = 4)
...
plot1 <- ggplot(mutateTimeEffectSize %>% 
                  filter(cat.alg1== "WeightedSum+old-BBC" & cat.alg2 == "WeightedSum"), aes(x = paste(cat.alg1), y = VD.estimate)) +
  theme(text = element_text(size=18),axis.text.x = element_text(size=15)) +
  geom_boxplot() +
  stat_summary(fun.y=mean,pch=22, size = 3, geom='point') +
  ylab("effect size") + xlab("")
...
plot2 <- ggplot(mutateTimeEffectSize %>% 
                  filter(cat.alg1== "WeightedSum+old-BBC" & cat.alg2 == "WeightedSum"), aes(x = paste(cat.alg1), y = improvement)) +
  theme(text = element_text(size=18),axis.text.x = element_text(size=15)) +
  geom_boxplot() +
  stat_summary(fun.y=mean,pch=22, size = 3, geom='point') +
  ylab("Improvement (perc)") + xlab("")
...
p  <- grid.arrange(plot1, plot2, ncol=2)
ggsave("figures/improvement-efficiency-ws-oldBBCvsnone.pdf", p, width = 7, height = 4)
...
plot1 <- ggplot(mutateTimeEffectSize %>% 
                  filter(cat.alg1== "WeightedSum+BBC" & cat.alg2 == "WeightedSum+old-BBC"), aes(x = paste(cat.alg1), y = VD.estimate)) +
  theme(text = element_text(size=18),axis.text.x = element_text(size=15)) +
  geom_boxplot() +
  stat_summary(fun.y=mean,pch=22, size = 3, geom='point') +
  ylab("effect size") + xlab("")
...
plot2 <- ggplot(mutateTimeEffectSize %>% 
                  filter(cat.alg1== "WeightedSum+BBC" & cat.alg2 == "WeightedSum+old-BBC"), aes(x = paste(cat.alg1), y = improvement)) +
  theme(text = element_text(size=18),axis.text.x = element_text(size=15)) +
  geom_boxplot() +
  stat_summary(fun.y=mean,pch=22, size = 3, geom='point') +
  ylab("Improvement (perc)") + xlab("")
...
p  <- grid.arrange(plot1, plot2, ncol=2)
ggsave("figures/improvement-efficiency-ws-oldBBCvsbbc.pdf", p, width = 7, height = 4)
